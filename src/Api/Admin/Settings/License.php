...
class License extends AbstractApi implements ApiInterface
{
    ...
    public function activate(WP_REST_Request $wprestRequest) : WP_REST_Response
    {
        ...
        if ($new_license_key === '') {
            return new WP_REST_Response(['message' => 'License key is empty'], 400);
        }
        $plugin_updater = Plugin::get_instance()->plugin_updater;
        $response = $plugin_updater->activate($new_license_key);
        if (\is_wp_error($response) || \wp_remote_retrieve_response_code($response) !== 200) {
            return new WP_REST_Response(['message' => \is_wp_error($response) ? $response->get_error_message() : 'An error occurred, please try again.'], 500);
        }
        ...
        return new WP_REST_Response(['message' => 'Plugin license key activated successfully', 'license' => $this->get_license()]);
    }
    public function deactivate(WP_REST_Request $wprestRequest) : WP_REST_Response
    {
        ...
        return new WP_REST_Response(['message' => 'Plugin license key de-activated successfully', 'license' => $this->get_license()]);
    }
